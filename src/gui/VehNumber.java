...
public class VehNumber extends SucemFrameWithTimer {
    ...
    private void jbInit() throws Exception {
        ...
        HPHML.setFont(new java.awt.Font("Dialog", Font.BOLD, 60));
        //jLabel1.setText("123456");
        HPHML.setBounds(new Rectangle(253, 65, 287, 54));
    ...
    public void jButton_0_actionPerformed(ActionEvent e) {
        String Number = "";
        Number = HPHML.getText();
        Number = Number + "0";
        if (Number.length() > 6) {
            Number = Number.substring(0, 6);
        }
        HPHML.setText(Number);
    }
    ...
    public void jButton_QX_actionPerformed(ActionEvent e) {
        ...
        HPHML.setText(Number.substring(0, Number.length() - 1));
    }

    public void jButton_XYB_actionPerformed(ActionEvent e) {
        Regexplain regex=new Regexplain("[A-Z]{1}[A-Z0-9]{1}[A-Z0-9]{1}[0-9]{2,3}");
        if(!regex.IsMatch(HPHML.getText())){
            HPHML.setText("");
            return;
        }
        Param.getInstance().setHPHM(HPHML.getText());
        this.setVisible(false);
        switch (Param.getInstance().GetSearchType()) {
        case Param.YWQK:
            VehResult vehResult = new VehResult(Param.getResultTime());
            vehResult.setVisible(true);
            break;
        case Param.CLWZ:
            VehWZ vehWz=new VehWZ(Param.getResultTime());
            vehWz.setVisible(true);
        }
        this.dispose();
    }
